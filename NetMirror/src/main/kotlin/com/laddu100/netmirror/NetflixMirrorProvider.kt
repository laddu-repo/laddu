...
class NetflixMirrorProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        Log.d("NetMirror", "Netflix: getMainPage START")
        if (cookie_value.isEmpty()) {
            cookie_value = bypass(netMirrorWorkingDomain)
            mainUrl = netMirrorWorkingDomain
        }
        Log.d("NetMirror", "Netflix: cookie=${if (cookie_value.isNotEmpty()) "YES" else "NO"} domain=$mainUrl")
        val cookies = mapOf(
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        Log.d("NetMirror", "Netflix: search START")
        if (cookie_value.isEmpty()) {
            cookie_value = bypass(netMirrorWorkingDomain)
            mainUrl = netMirrorWorkingDomain
        }
    ...
    override suspend fun load(url: String): LoadResponse? {
        Log.d("NetMirror", "Netflix: load START")
        if (cookie_value.isEmpty()) {
            cookie_value = bypass(netMirrorWorkingDomain)
            mainUrl = netMirrorWorkingDomain
        }
            ...
            override fun intercept(chain: Interceptor.Chain): Response {
                ...
                if (url.contains("playlist.php") || url.contains(".m3u8") || url.contains(".js") || url.contains(".jpg") || url.contains(".ts")) {
                    ...
                    val newRequest = request.newBuilder()
                        .header("Cookie", baseCookie + "; ott=nf; hd=on")
