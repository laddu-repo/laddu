...
public class DeviceInfo {
    ...
    private static File getFileForStorageType(StorageType storageType) {
        int i6 = AnonymousClass1.$SwitchMap$com$unity3d$services$core$api$DeviceInfo$StorageType[storageType.ordinal()];
        if (i6 != 1) {
            if (i6 != 2) {
        ...
        if (sensorList != null) {
            for (Sensor sensor : sensorList) {
                ...
                try {
                    ...
                    jSONObject.put(HandleInvocationsFromAdViewer.KEY_AD_TYPE, sensor.getType());
    ...
    public static void getSupportedAbis(WebViewCallback webViewCallback) {
        ...
        int i6 = 0;
        while (i6 < size) {
            String str = supportedAbis.get(i6);
            i6++;
            jSONArray.put(str);
        }
        webViewCallback.invoke(jSONArray);
    ...
    public static void getTimeZoneOffset(WebViewCallback webViewCallback) {
        webViewCallback.invoke(Integer.valueOf(TimeZone.getDefault().getOffset(System.currentTimeMillis()) / UnityAdsConstants.SafeGuards.InitRequestTimeoutPolicy.MIN_TIMEOUT));
    }
