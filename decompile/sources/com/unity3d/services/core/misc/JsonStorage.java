...
public class JsonStorage implements IJsonStorageReader {
    ...
    private synchronized void createObjectTree(String str) {
        ...
        for (int i6 = 0; i6 < split.length; i6++) {
            if (!jSONObject.has(split[i6])) {
                try {
                    jSONObject = jSONObject.put(split[i6], new JSONObject()).getJSONObject(split[i6]);
                } catch (Exception e10) {
    ...
    private synchronized Object findObject(String str) {
        ...
        for (int i6 = 0; i6 < split.length; i6++) {
            ...
            try {
                ...
                DeviceLog.exception("Couldn't read JSONObject: " + split[i6], e10);
                return null;
            }
        }
        return jSONObject;
